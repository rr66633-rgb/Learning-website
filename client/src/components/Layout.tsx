...
import { BeroChatbot } from "./BeroChatbot";
...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-white/95 backdrop-blur-md border-t border-[var(--border)] px-4 py-3 flex items-center gap-3">
        <Link
          href="/contact"
          className="flex-1 flex items-center justify-center gap-2 bg-[var(--green-primary)] text-white py-3 rounded-full text-sm font-bold active:scale-[0.97] transition-transform"
        >
          <CalendarCheck className="w-4 h-4" />
          سجّل الآن
        </Link>
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="w-12 h-12 bg-[#25D366] rounded-full flex items-center justify-center shadow-md active:scale-95 transition-transform"
          aria-label="واتساب"
        >
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-6 left-6 z-50 w-14 h-14 bg-[#25D366] rounded-full hidden md:flex items-center justify-center shadow-lg hover:scale-110 transition-transform active:scale-95 whatsapp-pulse"
        aria-label="واتساب"
      >
